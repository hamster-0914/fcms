...
import re
from fcmsapp.models import Staff
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            if options["interactive"]:
                ...
                name = self.get_input_data(field=None, message='Enter name: ')
                while True:
                    gender = self.get_input_data(field=None, message='Gender [Male/Female]: ')
                    if (gender.lower() != 'male') and (gender.lower() != 'female'):
                        self.stderr.write("Error: Please enter 'Male' or 'Female'.")
                        continue
                    else:
                        break
                while True:
                    dob = self.get_input_data(field=None, message='Enter date of birth [YYYY-MM-DD]: ')
                    try:
                         dob = datetime.strptime(dob, '%Y-%m-%d')
                         if dob > datetime.now():
                              self.stderr.write("Error: Please enter a valid date.")
                              continue
                    except ValueError:
                         self.stderr.write("Error: Please enter a valid date.")
                         continue
                    break
                while True:
                    phone_number = self.get_input_data(field=None, message='Enter phone number [01xxxxxxxx(x)]: ')
                    if re.match(r'^01\d{8,9}$', phone_number):
                         break
                    else:
                         self.stderr.write("Error: Please enter a valid phone number in the format 01xxxxxxxx or 01xxxxxxxxx.")
                         continue
    ...
    def get_input_data(self, field, message, default=None):
        ...
        return val if val else raw_value
